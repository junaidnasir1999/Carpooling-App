...
const DriverTermsAndConditions = ({navigation, route}) => {
  ...
  console.log(`${driverForm.firstName} ${driverForm.lastName}`);
  ...
  const handleSignup = () => {
    if (isChecked) {
      setLoading(true)
      auth()
        .createUserWithEmailAndPassword(driverForm.email, driverForm.password)
        .then(({user}) => {
    ...
    <View style={styles.mainContainer}>
      <View style={styles.scrollContainer}>
        ...
        <View style={styles.bottomContainer}>
          <Text
            // onPress={() => navigation.navigate('DriverPdf')}
            style={{
              fontSize: hp('2.1'),
              color: 'black',
              fontWeight: 'bold',
            ...
            Please <Text style={{color: colors.themeBlue}}>Click here</Text> to
            read the{' '}
            <Text
              style={{
